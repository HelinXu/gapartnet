...
LastEditTime: 2022-09-15 22:24:04
Description: 
'''
...
if __name__ == '__main__':
    ...
    # drawer3
    for i in range(len(CAT['drawer'][3]['ids'])):
        cat = 'drawer'
        idx = CAT[cat][3]["ids"][i]
        file = f'processed/{cat}/{idx}/motion_sapien.urdf'
        node, path = load_urdf(file)
        ...
        new_joint.append(ET.Element('parent', link='root'))
        new_joint.append(ET.Element('child', link='base_link'))
        new_joint.append(ET.Element('origin', xyz='0 0 0', rpy='1.57079633 3.14159265359 1.57079633'))
        node.append(new_joint)
        # save the etree
        with open(file.replace('sapien.urdf', 'sapien_v2.urdf'), 'wb') as f:
            f.write(ET.tostring(node, pretty_print=True))
    ...
    # drawer6
    for i in range(len(CAT['drawer'][6]['ids'])):
        cat = 'drawer'
        idx = CAT[cat][6]["ids"][i]
        file = f'processed/{cat}/{idx}/motion_sapien.urdf'
        node, path = load_urdf(file)
        ...
        new_joint.append(ET.Element('parent', link='root'))
        new_joint.append(ET.Element('child', link='base_link'))
        new_joint.append(ET.Element('origin', xyz='0 0 0', rpy='3.14159265359 3.14159265359 0'))
        node.append(new_joint)
        # save the etree
        with open(file.replace('sapien.urdf', 'sapien_v2.urdf'), 'wb') as f:
            f.write(ET.tostring(node, pretty_print=True))
