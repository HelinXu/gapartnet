...
LastEditTime: 2022-09-15 23:55:40
Description: 
'''
from dis import dis
import logging
...
# trashcan 1
def trashcan1(idx):
    def view_hinge_lid_bbox(asset: sapien.KinematicArticulation, display=False):
        for link in asset.get_links():
            linkname = link.get_name()
            if linkname != 'base_link' and linkname != 'root':
    ...
    urdf_path = f'processed/trashcan/{idx}/motion_sapien_v2.urdf'
    ic(urdf_path)
    ...
    if os.path.isfile(f'processed/trashcan/{idx}/init_pose.json'):
        with open(f'processed/trashcan/{idx}/init_pose.json', 'r') as f:
            data = json.load(f)
    ...
    while not viewer.closed:
        ...
        #     asset.set_root_pose(root_pose)
            ...
            bbox_points, bbox_lines = view_hinge_lid_bbox(asset, display=True)
            add_line_set_to_renderer(scene=scene, renderer=renderer, position=bbox_points, connection=bbox_lines)
        elif viewer.window.key_down('enter'):
            # write the pose to json file
            with open(f'processed/trashcan/{idx}/init_pose.json', 'w') as f:
                pose_dict = {
...
if __name__ == '__main__':
    for i in range(len(CAT['trashcan'][1]['ids'])):
        logging.info(CAT['trashcan'][1]['ids'][i])
        trashcan1(CAT['trashcan'][1]['ids'][i])
        logging.info(CAT['trashcan'][1]['ids'][i], 'done')